...
export default function PracticeHub() {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><header className="bg-[var(--deep-navy)] px-6 py-5 text-white"><div className="mx-auto flex max-w-6xl items-center justify-between"><Link href={`/dashboard?track=${track}`} className="text-xl font-black">NursePrep<span className="text-[var(--aqua)]">IQ</span></Link><div className="flex items-center gap-4"><span className="rounded-full bg-white/10 px-3 py-1 text-sm font-bold">NCLEX-{track.toUpperCase()}</span><Link href={`/dashboard?track=${track}`} className="text-sm font-bold text-cyan-100 hover:text-white">Dashboard</Link></div></div></header><section className="mx-auto max-w-6xl px-6 py-12"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="font-black tracking-wider text-[var(--teal)]">PRACTICE QUESTIONS</p><h1 className="mt-2 text-4xl font-black text-[var(--deep-navy)]">Choose what you want to strengthen.</h1><p className="mt-3 max-w-2xl text-lg text-slate-600">Practice by nursing area while NursePrepIQ tracks your performance and helps expose the topics that need more attention.</p></div><div className="flex gap-3"><div className="rounded-2xl bg-white px-5 py-3 shadow-sm ring-1 ring-slate-200"><p className="text-xs font-bold uppercase text-slate-500">Answered</p><p className="text-2xl font-black text-[var(--deep-navy)]">{loading ? '—' : stats.total}</p></div><div className="rounded-2xl bg-white px-5 py-3 shadow-sm ring-1 ring-slate-200"><p className="text-xs font-bold uppercase text-slate-500">Accuracy</p><p className="text-2xl font-black text-[var(--teal)]">{loading ? '—' : `${stats.accuracy}%`}</p></div></div></div><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{categories.map(category => { const questionCount=bank.filter(q=>q.discipline===category.name).length; const available=questionCount>0; return <article key={category.name} className={`rounded-[1.5rem] bg-white p-6 shadow-sm ring-1 ${category.featured ? 'ring-cyan-300' : 'ring-slate-200'}`}><div className="flex items-start justify-between"><span className="text-3xl" aria-hidden>{category.icon}</span><span className={`rounded-full px-3 py-1 text-xs font-black ${available?'bg-cyan-50 text-[var(--teal)]':'bg-slate-100 text-slate-500'}`}>{questionCount} Q</span></div><h2 className="mt-5 text-xl font-black text-[var(--deep-navy)]">{category.name}</h2><p className="mt-2 min-h-12 text-sm leading-6 text-slate-600">{category.description}</p>{available ? <Link href={`/practice/library?discipline=${encodeURIComponent(category.name)}`} className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-[var(--deep-navy)] px-4 py-3 font-extrabold !text-white shadow-sm transition hover:bg-[var(--navy)] focus:outline-none focus:ring-2 focus:ring-[var(--aqua)] focus:ring-offset-2">Learn & Practice <span className="ml-2 text-[var(--aqua)]">→</span></Link> : <div className="mt-6 rounded-xl bg-slate-50 px-4 py-3 text-center text-sm font-bold text-slate-400">More questions coming next</div>}</article>; })}</div></section></main>;
}
