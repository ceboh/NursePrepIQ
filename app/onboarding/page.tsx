...
function OnboardingForm() {
  ...
  async function continueFlow() {
    ...
    const { error } = await supabase.from("profiles").upsert({
      id: user.id, email: user.email, exam_track: track.toLowerCase(), exam_date: examDate || null,
      test_timing: timing, confidence_level: confidence, daily_goal: dailyGoal, onboarding_complete: true,
    });
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-10"><div className="mx-auto max-w-2xl"><a href="/" className="text-xl font-extrabold text-[var(--navy)]">NursePrep<span className="text-[var(--teal)]">IQ</span></a><div className="mt-8 rounded-3xl bg-white p-7 shadow-sm ring-1 ring-slate-200 md:p-10">
    ...
    <p className="mt-6 text-sm font-bold text-[var(--teal)]">{goalsStep ? "PERSONALIZE YOUR PLAN" : `YOUR ${track} STUDY PATH`}</p>
    <h1 className="mt-2 text-3xl font-black text-[var(--navy)]">{goalsStep ? `Set your NCLEX-${track} study goals.` : `Let’s personalize your NCLEX-${track} preparation.`}</h1>
    ...
    {message && <p className="mt-5 rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-700">{message}</p>}<button type="button" onClick={continueFlow} disabled={!timing||!confidence||loading} className="mt-8 w-full rounded-xl bg-[var(--navy)] py-4 font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{loading?"Saving your plan…":goalsStep?`Build My ${track} Dashboard →`:`Continue with NCLEX-${track} →`}</button>
  </div></div></main>;
}
